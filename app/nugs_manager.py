...
def run_full_metadata_update():
    ...
    with Database() as db:
        in_progress = db.query("SELECT refresh_in_progress FROM nugs_metadata")[0]

        if in_progress:
            logging.info("Skipping update.")
            return

        db.execute('UPDATE nugs_metadata SET refresh_in_progress = TRUE')
    ...
    with Database() as db:
        ...
        db.cursor.executemany('INSERT INTO nugs_items (date_text, url) VALUES (%s, %s)', result_iterator())
        db.execute('UPDATE nugs_metadata SET last_refresh_date = now(), refresh_in_progress = FALSE ')
...
def nugs_recording_url(date: str) -> str:
    with Database() as db:
        try:
            return db.query('SELECT url FROM nugs_items WHERE date_text = %s', (date,))[0]
        except IndexError:
            return ''
